...
export default function AnonymousEditForm({
  ...
  const handleSubmit = async (token = verificationToken) => {
    ...
    console.log('[AnonymousEdit] handleSubmit called with token:', token ? 'present' : 'missing', 'verificationToken state:', verificationToken ? 'present' : 'missing');

    if (!token) {
      // Need to verify email first
      console.log('[AnonymousEdit] No token available, requesting email verification');
      await handleSendVerification();
      return;
    }

    console.log('[AnonymousEdit] Proceeding with submission using verification token');
    ...
    try {
      ...
      if (result.success) {
        // DO NOT clear cached token - it should remain valid for 24 hours
        // This allows users to make multiple edits without re-verifying
        // Token expiration is handled by getCachedVerificationToken()
