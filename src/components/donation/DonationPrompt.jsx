 ...
 */
const DonationPrompt = ({ onClose, onDonate, messages = null, MascotComponent = null }) => {
  ...
  // Handle mascot exit and card entrance
  useEffect(() => {
    if (stage === 'exiting') {
      ...
      {stage !== 'card' && (
        <div className="relative pointer-events-auto flex items-end gap-4">
          ...
          >
            {MascotComponent ? (
              ...
              <div className="relative w-24 h-24 flex items-center justify-center">
                <Heart
                  className="w-20 h-20 text-red-500 fill-red-500 animate-pulse"
                  style={{
          ...
          {speechBubbleVisible && (
            ...
            >
              <div className="relative bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-xl border-2 border-red-500 dark:border-red-400 max-w-xs">
                <div className="text-gray-800 dark:text-gray-200 font-medium">
                  {displayedText}
                  {messageIndexRef.current < message.length && (
                    <span className="inline-block w-1 h-4 ml-1 bg-red-500 animate-pulse" />
                  )}
                </div>

                {/* Speech bubble tail pointing left to mascot */}
                <div className="absolute -left-2 bottom-6 w-4 h-4 bg-white dark:bg-gray-800 border-l-2 border-b-2 border-red-500 dark:border-red-400 transform rotate-45" />
              </div>
            </div>
          )}
