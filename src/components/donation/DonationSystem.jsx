 ...
 *
 * @param {React.Component} MascotComponent - Optional custom mascot component (for game-specific characters)
 */
const DonationSystem = ({ MascotComponent = null }) => {
  const { config } = useWikiConfig();

  // Get donation config from wiki-config.json features.donation
  const donationConfig = config?.features?.donation || {};
  const promptConfig = donationConfig.prompt || {};

  // Map wiki config to hook config format
  const hookConfig = {
    enabled: donationConfig.enabled || false,
    cooldownHours: (promptConfig.daysUntilNextPrompt || 7) * 24, // Convert days to hours
    triggerChance: 0.3, // 30% chance to trigger
    minInteractionsBeforeEligible: promptConfig.minPagesViewed || 2,
  };

  const { shouldShow, handleClose, handleDonate, triggerPrompt } = useDonationPrompt(hookConfig);
